...
namespace Cobra.RobotPanel
{
    ...
    public partial class MainControl
    {
        ...
        {
            mDataGrid.PreviewMouseLeftButtonDown += MDataGrid_PreviewMouseLeftButtonDown;
            mDataGrid.Drop += MDataGrid_Drop;
        }

        private void MDataGrid_Drop(object sender, DragEventArgs e)
        {
            if (rowIndex < 0)
                return;
            ...
            if (index == rowIndex)
                return;
            if (index == mDataGrid.Items.Count - 1)
            {
                MessageBox.Show("This row-index cannot be drop");
                return;
            }
            Model changedProduct = viewmode.robot_commands[rowIndex];
            viewmode.robot_commands.RemoveAt(rowIndex);
            viewmode.robot_commands.Insert(index, changedProduct);
        }

        private void MDataGrid_PreviewMouseLeftButtonDown(object sender, MouseButtonEventArgs e)
        {
            DataGrid dg = sender as DataGrid;
            DataGridCellInfo dgc = dg.CurrentCell;
            DataGridColumn column = dgc.Column;
            if (column != null)
            {
                string header = column.Header as string;
                if (string.Compare(header, "ID") != 0) return;
            }
            rowIndex = GetCurrentRowIndex(e.GetPosition);
            if (rowIndex < 0)
                return;
            mDataGrid.SelectedIndex = rowIndex;
            Model selectedEmp = mDataGrid.Items[rowIndex] as Model;
            if (selectedEmp == null)
                return;
            DragDropEffects dragdropeffects = DragDropEffects.Move;
            if (DragDrop.DoDragDrop(mDataGrid, selectedEmp, dragdropeffects) != DragDropEffects.None)
                mDataGrid.SelectedItem = selectedEmp;
        }
        ...
        {
            if (mDataGrid.ItemContainerGenerator.Status != GeneratorStatus.ContainersGenerated)
                return null;
            return mDataGrid.ItemContainerGenerator.ContainerFromIndex(index) as DataGridRow;
        }

        private int GetCurrentRowIndex(GetPosition pos)
        {
            int curIndex = -1;
            for (int i = 0; i < mDataGrid.Items.Count; i++)
            {
