...
namespace Cobra.MCUConfigurationPanel
{
    public class SFLViewMode
    {
        ...
        private void InitSFLParameter(Parameter param)
        {
            ...
            model.bwone = true;
            model.bsubmenu = true;
           // model.address = (param.reglist[0].address).ToString();
            Reg reg = null;
            foreach (KeyValuePair<string, Reg> dic in param.reglist)
            {
               ...
               // model.reg.address
                ...
                model.address += reg.address.ToString("X2")+dic.Key;
            ...
            foreach (DictionaryEntry de in param.sfllist[sflname].nodetable)
            {
                switch (de.Key.ToString())
                {
                    case "NickName":
                        ...
                        //model.parent.reglist[0].address
                        break;
                    case "Order":
                        {/*
                    ...
                    case "DefValue":
                        {
                            model.sphydata = de.Value.ToString();
                            if (!Double.TryParse(de.Value.ToString(), out ddata))
                                model.data = 0.0;
                            else
                ...
                case 0:
                    {
                        ...
                        device_parent.UpdataDEMParameterList(param.parent);
                        break;
                    }
                case 1:
                    {
                ...
                case "itemlist":
                    {
                        ...
                        model.listindex = model.listindex;
                        model.errorcode = p.errorcode;
                        break;
                    }
                case "sphydata":
