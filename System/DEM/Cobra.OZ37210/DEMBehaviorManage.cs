    ...
    internal class DEMBehaviorManage
    {
        ...
        protected UInt32 ReadWord(byte reg, ref UInt16 pval)
        {
            ...
            lock (m_lock)
            {
                ...
                if (((reg >= 0x41) && (reg <= 0x4A)) || ((reg >= 0x61) && (reg <= 0x6A)))
                    FolderMap.WriteFile(string.Format("{0:x2}", reg));
            }
            return ret;
        }
        ...
        #region EFUSE寄存器子级操作
        protected UInt32 OnWorkMode(ElementDefine.COBRA_AZALEA5D_WKM wkm)
        {
            byte blow = 0;
            byte bhigh = 0;
            UInt16 wdata = 0;
            ...
            if (wkm == ElementDefine.COBRA_AZALEA5D_WKM.EFUSE_WORKMODE_NORMAL)
            {
        ...
        #region EFUSE功能操作
        protected UInt32 WorkMode(ElementDefine.COBRA_AZALEA5D_WKM wkm)
        {
            UInt32 ret = LibErrorCode.IDS_ERR_SUCCESSFUL;
            lock (m_lock)
            {
        ...
        public UInt32 Read(ref TASKMessage msg)
        {
            ...
            if (string.Equals(msg.gm.sflname, "TestScan"))
            {
                for (byte i = 0x41; i < 0x4B; i++)
                {
                    ret = ReadWord(i, ref wdata);
                    parent.m_OpRegImg[i].err = ret;
                    parent.m_OpRegImg[i].val = wdata;
                    ret = ReadWord((byte)(i + 0x20), ref wdata);
                    parent.m_OpRegImg[i + 0x20].err = ret;
                    parent.m_OpRegImg[i + 0x20].val = wdata;
                }
                foreach (byte badd in OpReglist)
                {
                    if (((badd >= 0x41) && (badd <= 0x4A)) || ((badd >= 0x61) && (badd <= 0x6A)))
                        continue;
        ...
        public UInt32 Write(ref TASKMessage msg)
        {
            ...
            if (EFUSEReglist.Count != 0)
            {
                ...
                ret = WorkMode(ElementDefine.COBRA_AZALEA5D_WKM.EFUSE_WORKMODE_PROGRAM);
                if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
                ...
                ret = WorkMode(ElementDefine.COBRA_AZALEA5D_WKM.EFUSE_WORKMODE_NORMAL);
                if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
            }
            ...
            {
                ret = WorkMode(ElementDefine.COBRA_AZALEA5D_WKM.EFUSE_WORKMODE_WRITE_MAP_CTRL);
                if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
        ...
        private UInt32 CountSlope(ParamContainer demparameterlist)
        {
            ...
            for (int i = 0; i < demparameterlist.parameterlist.Count; i++)
            {
                ...
                else if(param.guid == ElementDefine.VBATT_Slope_Trim)
        ...
        public UInt32 GetSystemInfor(ref TASKMessage msg)
        {
            ...
            if (demparameterlist == null) return ret;
            //Cell Number
            ret = ReadWord(0x9F, ref wval); //Cell number
            if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;

            bval = (byte)((wval & 0x7000) >> 12);
            if (bval <= 2) bval = 5;
            else bval += 3;
        ...
        public UInt32 GetThmCrrtSel(ref int thm_crrt)
        {
            ...
            ret = ReadWord(ElementDefine.THM0_ADC_1T_Reg, ref wval);
            if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
            parent.m_OpRegImg[ElementDefine.THM0_ADC_1T_Reg].val = wval;
            parent.m_OpRegImg[ElementDefine.THM0_ADC_1T_Reg].err = ret;

            ret = ReadWord(ElementDefine.THM1_ADC_1T_Reg, ref wval);
            if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
            parent.m_OpRegImg[ElementDefine.THM1_ADC_1T_Reg].val = wval;
            parent.m_OpRegImg[ElementDefine.THM1_ADC_1T_Reg].err = ret;
