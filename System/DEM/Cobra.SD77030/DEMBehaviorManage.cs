...
namespace Cobra.SD77030
{
    internal class DEMBehaviorManage
    {
        ...
        public UInt32 Read(ref TASKMessage msg)
        {
            ...
            if (EfuseMapReglist.Count == 1) //避免后期增减参数，所以目前为0x1C个，暂设为0x10
            ...
            if (EfuseMapReglist.Count != 0)
            {
                foreach (byte badd in EfuseMapReglist)
                {
                    ...
                    parent.m_MapRegImg[badd].val = wdata;
                }
            }
        ...
        public UInt32 Write(ref TASKMessage msg)
        {
            ...
            if (YFLASHReglist.Count != 0)
            {
                foreach (byte badd in YFLASHReglist)
                {
                    ret = YFLASHWriteWord(badd, parent.m_YFRegImg[badd].val);
                    parent.m_YFRegImg[badd].err = ret;
                }
            }
            if (EfuseMapReglist.Count != 0)
            {
                ...
                foreach (byte badd in EfuseMapReglist)
                {
                    ret = WriteWord(badd, parent.m_MapRegImg[badd].val);
                    parent.m_MapRegImg[badd].err = ret;
                }
            }
        ...
        public UInt32 ConvertHexToPhysical(ref TASKMessage msg)
        {
            ...
            List<Parameter> EfuseMapParamlist = new List<Parameter>();
            ...
            if (EfuseMapParamlist.Count != 0)
            {
                for (int i = 0; i < EfuseMapParamlist.Count; i++)
                {
                    param = (Parameter)EfuseMapParamlist[i];
                    if (param == null) continue;
                    if ((param.guid & ElementDefine.ElementMask) == ElementDefine.TemperatureElement) continue;
