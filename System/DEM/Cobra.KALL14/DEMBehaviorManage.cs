...
namespace Cobra.KALL14
{
    internal class DEMBehaviorManage
    {
        ...
        protected UInt32 TriggerScanRequest()
        {
            UInt32 ret = LibErrorCode.IDS_ERR_SUCCESSFUL;
            lock (m_lock)
            {
                ret = OnTriggerScanRequest();
            }
            return ret;
        }
        ...
        protected UInt32 OnGetCellBalance(ref ElementDefine.CELL_BALANCE cb)
        {
            UInt32 ret = LibErrorCode.IDS_ERR_SUCCESSFUL;
            ushort buf = 0;
            ret = OnReadWord(ElementDefine.CB_OPTION_OFFSET, ref buf);
            buf &= 0x6000;
            buf >>= 13;
        ...
        protected UInt32 OnSetCellBalance(ElementDefine.CELL_BALANCE cb)
        {
            ...
            ret = OnWriteWord(ElementDefine.CB_OPTION_OFFSET, buf);
            return ret;
        }
        ...
        private UInt32 SafeRead(ref ushort curr)
        {
            UInt32 ret = LibErrorCode.IDS_ERR_SUCCESSFUL;
            byte pec_error = 0;
            for (; pec_error < 4; pec_error++)
            {
                ret = OnReadWord((byte)ElementDefine.CURRENT_OFFSET, ref curr);
        ...
        protected UInt32 OnTriggerScanRequest()
        {
            UInt32 ret = LibErrorCode.IDS_ERR_SUCCESSFUL;
            ushort buf = 0, curr1 = 0;
            byte pec_error = 0;
            ret = OnWriteWord(0x43, 0x4000);
            if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL)
                return ret;
            ...
            {
                ret = OnReadWord(0x43, ref buf);
                if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL)
                    return ret;

                if ((buf & 0x4000) == 0x4000)
                {

                    ret = SafeRead(ref curr1);
                    if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL)
                        return ret;
                    ...
                    parent.m_OpRegImg[ElementDefine.CURRENT_OFFSET].err = ret;
                    parent.m_OpRegImg[ElementDefine.CURRENT_OFFSET].val = curr1;
                    return ret;
                }
                else
                {
        ...
        public UInt32 Read(ref TASKMessage msg)
        {
            ...
            if (msg.gm.sflname == "Scan" || msg.gm.sflname == "Trim")  //如果是Scan/Trim SFL
            {
                ...
                ret = ReadWord(ElementDefine.CB_SEL_OFFSET, ref wdata);
                if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL)
                    return ret;
                parent.m_OpRegImg[ElementDefine.CB_SEL_OFFSET].err = ret;
                parent.m_OpRegImg[ElementDefine.CB_SEL_OFFSET].val = wdata;
                ...
                foreach (byte badd in OpReglist)
                {
                    ...
                    //}
                    //else
                    if(badd != ElementDefine.CB_SEL_OFFSET)
                    {
        ...
        */
        ...
        public UInt32 Write(ref TASKMessage msg)
        {
            ...
            UInt16[] EFUSEATEbuf = new UInt16[7];
            List<byte> EFUSEUSRReglist = new List<byte>();
            UInt16[] EFUSEUSRbuf = new UInt16[9];
            List<byte> OpReglist = new List<byte>();
        ...
        private uint ReadAvrage(ref TASKMessage msg)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                {
                    if (param.guid == ElementDefine.Current)
                        param.phydata = -param.phydata * parent.etrx / 1000.0F;
                }
        ...
        public UInt32 Command(ref TASKMessage msg)
        {
            ...
            switch ((ElementDefine.COMMAND)msg.sub_task)
            {
                ...
                case ElementDefine.COMMAND.TESTCTRL_SLOP_TRIM:
                    {
                        ...
                        {
                            //Write Slope Trim
                            WriteWord(0x72, (ushort)((code << 13) | (code << 10) | (code << 7) | (code << 4) | code));
                            WriteWord(0x73, (ushort)((code << 13) | (code << 10) | (code << 7) | (code << 4) | code));
                            WriteWord(0x74, (ushort)((code << 13) | (code << 10) | (code << 7) | (code << 4) | code));
                        ...
                        for (ushort code = 8; code < 16; code++)
                        {
                            //Write Slope Trim
                            WriteWord(0x72, (ushort)(code));
                            WriteWord(0x73, (ushort)(code));
                            WriteWord(0x74, (ushort)(code));
                            ...
                            {
                                param = demparameterlist.parameterlist[i];
                                if (param.guid == ElementDefine.Cell1 || param.guid == ElementDefine.Cell2 || param.guid == ElementDefine.Current)
                                {
                                    param.sphydata += param.phydata.ToString() + ",";
                                }
                            }
                ...
                case ElementDefine.COMMAND.ATE_EMPTY_CHECK:
                    ret = ATEEmptyCheck();
                    if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL)
                        return ret;
                    break;
        ...
        private uint ATEEmptyCheck()
        {
            UInt32 ret = LibErrorCode.IDS_ERR_SUCCESSFUL;
            ushort pval = 0;
            for (byte index = (byte)ElementDefine.OP_ATE_OFFSET; index <= (byte)ElementDefine.OP_ATE_TOP; index++)
            {
            ...
            return ElementDefine.IDS_ERR_DEM_ATE_EMPTY_CHECK_FAILED;
        }
        ...
        public UInt32 EpBlockRead()
        {
            ...
            if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL)
                return ret;
            ret = WriteWord(0x40, (ushort)(wdata | 0x0080));
            if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL)
                return ret;
            return ret;
        }
