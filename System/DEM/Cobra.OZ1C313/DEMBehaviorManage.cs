...
namespace Cobra.MissionPeak
{
    internal class DEMBehaviorManage
    {
        ...
        public UInt32 Read(ref TASKMessage msg)
        {
            ...
            List<byte> OTPReglist = new List<byte>();
            ...
            OTPReglist = OTPReglist.Distinct().ToList();
            OpReglist = OpReglist.Distinct().ToList();

            //Read 
            if (OTPReglist.Count != 0)
            {
                return LibErrorCode.IDS_ERR_SUCCESSFUL;
            }
        ...
        public UInt32 ConvertHexToPhysical(ref TASKMessage msg)
        {
            ...
            List<Parameter> OTPParamList = new List<Parameter>();
            ...
            if (OTPParamList.Count != 0)
            {
                for (int i = 0; i < OTPParamList.Count; i++)
                {
                    param = (Parameter)OTPParamList[i];
        ...
        public UInt32 Command(ref TASKMessage msg)
        {
            ...
            switch ((ElementDefine.COBRA_SUB_TASK)msg.sub_task)
            {
                ...
                case ElementDefine.COBRA_SUB_TASK.SUB_TASK_TEST_MODE:
                    {
                        ret = SwitchMode((ElementDefine.COBRA_SUB_TASK)msg.sub_task);
                        break;
                    }
                case ElementDefine.COBRA_SUB_TASK.SUB_TASK_START_TRIM: //Start Trim
                    {
                        ret = ReadByte(0x23, ref bdata);
                        if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
                        bdata |= 0x20;
                        ret = WriteByte(0x23, bdata);
                    }
                    break;
                case ElementDefine.COBRA_SUB_TASK.SUB_TASK_BURN: //BURN
                    {
                        ret = ReadByte(0x23, ref bdata);
                        if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
                        bdata |= 0x01;
                    ...
                    break;
                case ElementDefine.COBRA_SUB_TASK.SUB_TASK_FREEZE: //FREEZE
                    {
                        ret = ReadByte(0x1C, ref bdata);
                        if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
                        if ((bdata&0x01) == 0x01)
                        {
                            msg.gm.message = "Freeze bit as 1!!!";
                            msg.controlreq = COMMON_CONTROL.COMMON_CONTROL_WARNING;
                        }
                        else
                        {
                            msg.gm.message = "Freeze bit as 0!!!";
                            msg.controlreq = COMMON_CONTROL.COMMON_CONTROL_WARNING;
                        }
                    }
                    break;
        ...
        public UInt32 GetDeviceInfor(ref DeviceInfor deviceinfor)
        {
            ...
            ret = ReadByte(0x30, ref bval);
            if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;

            deviceinfor.status = 0;
            deviceinfor.shwversion = String.Format("{0:x2}", bval);

            ret = ReadByte(0x31, ref bval);
            if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;

            deviceinfor.type = bval;
            return ret;
        }
        ...
        public UInt32 SwitchMode(ElementDefine.COBRA_SUB_TASK subTask)
        {
            ...
            {
                case ElementDefine.COBRA_SUB_TASK.SUB_TASK_NORMAL_MODE:
                    ret = WriteByte(ElementDefine.TEST_MODE_Reg,ElementDefine.NORMAL_MODE_PWD);
                    break;
                case ElementDefine.COBRA_SUB_TASK.SUB_TASK_TEST_MODE:
                    ret = WriteByte(ElementDefine.TEST_MODE_Reg, ElementDefine.NORMAL_MODE_PWD);
                    if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
                    
                    ret = WriteByte(ElementDefine.TEST_MODE_Reg, ElementDefine.TEST_MODE_PWD);
                    if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
