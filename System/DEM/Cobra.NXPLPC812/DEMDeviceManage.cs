...
namespace Cobra.LPC812
{
    public class DEMDeviceManage : IDEMLib
    {
        ...
        internal COBRA_HWMode_Reg[] m_ProjParamImg = new COBRA_HWMode_Reg[ElementDefine.PARA_MEMORY_SIZE];
        ...
        {
            for (ushort i = 0; i < ElementDefine.PARA_MEMORY_SIZE; i++)
            {
                m_ProjParamImg[i] = new COBRA_HWMode_Reg();
                m_ProjParamImg[i].val = 0x00;
                m_ProjParamImg[i].err = LibErrorCode.IDS_ERR_SUCCESSFUL;
            }
        }
