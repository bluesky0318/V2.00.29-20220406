...
namespace Cobra.FWNewTon.G2
{
    internal class DEMBehaviorManage
    {
        ...
        protected UInt32 OnBlockWrite(ref TASKMessage msg)
        {
            ...
            if (startAddr > ElementDefine.SyseFlash_EndAddress) //Keep the old way to access chip
            {
                lock (m_lock)
                {
                    ret = OnWriteWord(0x02, ElementDefine.BLOCK_OPERATION_BYTES);
            ...
            else
            {
                ...
                {
                    Array.Clear(ElementDefine.m_temp_array, 0, ElementDefine.m_temp_array.Length);
                    Array.Copy(msg.flashData, m * 128, ElementDefine.m_temp_array, 0, ElementDefine.m_temp_array.Length);
                    //1. Write RAM,128bytes interna loop
                    for (UInt32 n = 0; n < 32; n++)
                    {
                        dWal = SharedFormula.MAKEDWORD(SharedFormula.MAKEWORD(ElementDefine.m_temp_array[n * 4 + 0], ElementDefine.m_temp_array[n * 4 + 1]),
                            SharedFormula.MAKEWORD(ElementDefine.m_temp_array[n * 4 + 2], ElementDefine.m_temp_array[n * 4 + 3]));
                ...
                {
                    Array.Clear(ElementDefine.m_temp_array, 0, ElementDefine.m_temp_array.Length);
                    Array.Copy(msg.flashData, quotient * 128, ElementDefine.m_temp_array, 0, remainder);
                    //1. Write RAM,128bytes interna loop
                    for (UInt32 n = 0; n < 32; n++)
                    {
                        dWal = SharedFormula.MAKEDWORD(SharedFormula.MAKEWORD(ElementDefine.m_temp_array[n * 4 + 0], ElementDefine.m_temp_array[n * 4 + 1]),
        ...
        protected UInt32 OnBlockWrite(UInt16 startAddr, byte[] buffer)
        {
            ...
            else
            {
                ushort addr = 0;
                UInt32 dWal = 0;
                int remainder = buffer.Length % 128;
                int quotient = buffer.Length / 128;

                //128 * n
                for (UInt32 m = 0; m < quotient; m++)
                {
                    Array.Clear(ElementDefine.m_temp_array, 0, ElementDefine.m_temp_array.Length);
                    Array.Copy(buffer, m * 128, ElementDefine.m_temp_array, 0, ElementDefine.m_temp_array.Length);
                    //1. Write RAM,128bytes interna loop
                    for (UInt32 n = 0; n < 32; n++)
                    {
                        dWal = SharedFormula.MAKEDWORD(SharedFormula.MAKEWORD(ElementDefine.m_temp_array[n * 4 + 0], ElementDefine.m_temp_array[n * 4 + 1]),
                ...
                {
                    Array.Clear(ElementDefine.m_temp_array, 0, ElementDefine.m_temp_array.Length);
                    Array.Copy(buffer, quotient * 128, ElementDefine.m_temp_array, 0, remainder);
                    //1. Write RAM,128bytes interna loop
                    for (UInt32 n = 0; n < 32; n++)
                    {
                        dWal = SharedFormula.MAKEDWORD(SharedFormula.MAKEWORD(ElementDefine.m_temp_array[n * 4 + 0], ElementDefine.m_temp_array[n * 4 + 1]),
                    ...
                    //2.Write the length
                    ret = OnBlockWordWrite(0x7033, 0x1F);
                    if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
                    //3.Trigger the programming
                    addr = (ushort)(startAddr + quotient * 128 / 4);
                    ret = OnBlockWordWrite(addr, 0);
        ...
        protected UInt32 OnBlock4BytesWrite(UInt16 startAddr, UInt32 wval)
        {
            ...
            {
                //1. Write RAM,128bytes interna loop
                ret = OnBlock4BytesWrite(0x83E0, wval);
                if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
                //2.Write the length
                ret = OnBlockWordWrite(0x703B, 0);
                if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
                //3.Trigger the programming
                ret = OnBlockWordWrite(startAddr, 0);
                if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
            }
            return ret;
        }
        ...
        public UInt32 ConvertHexToPhysical(ref TASKMessage msg)
        {
            ...
            if (ProjParamList.Count != 0)
            {
                for (int i = 0; i < ProjParamList.Count; i++)
                {
                    param = (Parameter)ProjParamList[i];
                    if (param == null) continue;
            ...
            if (m_Json_Options.ContainsKey("Button"))
            {
                ...
                switch (m_Json_Options["Button"])
                {
                    ...
                    case "FullDownloadPrj":
                        ...
                        ret = ret = ChipOperation(ElementDefine.CHIP_OPERA_MODE.SOFT_RESET);
                        if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
        ...
        private UInt32 MainBlockErase()
        {
            ...
            if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
            for (int n = 0; n < ElementDefine.RETRY_COUNTER * 5; n++)
            {
                Thread.Sleep(30);
        ...
        private UInt32 ParameterAreaErase()
        {
            ...
            wval = SharedFormula.MAKEWORD(0x00, 0x03);//0x3C);
            ret = BlockWordWrite(ElementDefine.I2C_Adress_PAGE_ERASE, wval);
            if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
        ...
        private UInt32 CheckSum(ElementDefine.COBRA_NEWTON_MEMORY_TYPE type, UInt32 startAddr, UInt32 endAddr, byte[] buf)
        {
            ...
            FolderMap.WriteFile(string.Format("CRC:HW-{0:X4},SW-{1:X4}", wval, sCrc));
        ...
        private UInt32 UnLockParameterArea()
        {
            ...
            if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
            ret = Block4BytesWrite(0x50F7, 0x00003DFE);
            if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
        ...
        private UInt32 LockParameterArea()
        {
            UInt32 ret = LibErrorCode.IDS_ERR_SUCCESSFUL;
            /*Array.Clear(ElementDefine.m_temp_array, 0, ElementDefine.m_temp_array.Length);
            ret = BlockRead(ElementDefine.InfoFlash_StartAddress + 0x60, ref ElementDefine.m_temp_array);
            if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
            ElementDefine.m_temp_array[127] = 0x89; ElementDefine.m_temp_array[126] = 0xAB;
            ElementDefine.m_temp_array[125] = 0xCD; ElementDefine.m_temp_array[124] = 0xEF;
            ElementDefine.m_temp_array[123] = 0x89; ElementDefine.m_temp_array[122] = 0xAB;
            ElementDefine.m_temp_array[121] = 0xCD; ElementDefine.m_temp_array[120] = 0xEF;
            ElementDefine.m_temp_array[119] = 0x89; ElementDefine.m_temp_array[118] = 0xAB;
            ElementDefine.m_temp_array[117] = 0xCD; ElementDefine.m_temp_array[116] = 0xEF;
            ElementDefine.m_temp_array[115] = 0x00; ElementDefine.m_temp_array[114] = 0x00;
            ElementDefine.m_temp_array[113] = 0x88; ElementDefine.m_temp_array[112] = 0x90;
            ElementDefine.m_temp_array[111] = 0x00; ElementDefine.m_temp_array[110] = 0x00;
            ElementDefine.m_temp_array[109] = 0x91; ElementDefine.m_temp_array[108] = 0xB4;
            ElementDefine.m_temp_array[107] = 0x00; ElementDefine.m_temp_array[106] = 0x00;
            ElementDefine.m_temp_array[105] = 0xB5; ElementDefine.m_temp_array[104] = 0xBC;
            ElementDefine.m_temp_array[99] = 0x89; ElementDefine.m_temp_array[98] = 0xAB;
            ElementDefine.m_temp_array[97] = 0xCD; ElementDefine.m_temp_array[96] = 0xEF;
            ElementDefine.m_temp_array[95] = 0x00; ElementDefine.m_temp_array[94] = 0x00;
            ElementDefine.m_temp_array[93] = 0x3D; ElementDefine.m_temp_array[92] = 0xFF;
            ret = BlockWrite(ElementDefine.InfoFlash_StartAddress + 0x60, ElementDefine.m_temp_array);
            if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;*/
            ret = Block4BytesWrite(0x50FF, 0x89ABCDEF);
            if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
            ret = Block4BytesWrite(0x50FE, 0x89ABCDEF);
        ...
        private UInt32 WriteSysInfoAreaCleareFlashFreshBit(ref byte[] buf)
        {
            UInt32 ret = LibErrorCode.IDS_ERR_SUCCESSFUL;
            /*
            ...
            if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;*/
            buf[0x100 * 2 - 1] = 0x89; buf[0x100 * 2 - 2] = 0xAB; buf[0x100 * 2 - 3] = 0xCD; buf[0x100 * 2 - 4] = 0xEF;
            //ret = Block4BytesWrite(0x50FF, 0x89ABCDEF);if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
            buf[0xFE * 2 - 1] = 0x89; buf[0xFE * 2 - 2] = 0xAB; buf[0xFE * 2 - 3] = 0xCD; buf[0xFE * 2 - 4] = 0xEF;
            //ret = Block4BytesWrite(0x50FE, 0x89ABCDEF);if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
            buf[0xFC * 2 - 1] = 0x89; buf[0xFC * 2 - 2] = 0xAB; buf[0xFC * 2 - 3] = 0xCD; buf[0xFC * 2 - 4] = 0xEF;
            //ret = Block4BytesWrite(0x50FD, 0x89ABCDEF);if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
            //ret = Block4BytesWrite(0x50FC, 0x00008890);
            buf[0xFA * 2 - 1] = 0x00; buf[0xFA * 2 - 2] = 0x00; buf[0xFA * 2 - 3] = 0x80; buf[0xFA * 2 - 4] = 0x90;
            //ret = Block4BytesWrite(0x50FC, 0x00008090);if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
            buf[0xF8 * 2 - 1] = 0x00; buf[0xF8 * 2 - 2] = 0x00; buf[0xF8 * 2 - 3] = 0x91; buf[0xF8 * 2 - 4] = 0xB4;
            //ret = Block4BytesWrite(0x50FB, 0x000091B4);if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
            buf[0xF6 * 2 - 1] = 0x00; buf[0xF6 * 2 - 2] = 0x00; buf[0xF6 * 2 - 3] = 0xB5; buf[0xF6 * 2 - 4] = 0xBC;
            //ret = Block4BytesWrite(0x50FA, 0x0000B5BC); if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
            buf[0xF2 * 2 - 1] = 0x89; buf[0xF2 * 2 - 2] = 0xAB; buf[0xF2 * 2 - 3] = 0xCD; buf[0xF2 * 2 - 4] = 0xEF;
            //ret = Block4BytesWrite(0x50F8, 0x89ABCDEF);if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
            buf[0xF0 * 2 - 1] = 0x00; buf[0xF0 * 2 - 2] = 0x00; buf[0xF0 * 2 - 3] = 0x3D; buf[0xF0 * 2 - 4] = 0xFF;
            //ret = Block4BytesWrite(0x50F7, 0x00003DFF);if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
            return ret;
        }
        ...
        private UInt32 RemapEflash()
        {
            UInt16 wval = 0;
            UInt32 ret = LibErrorCode.IDS_ERR_SUCCESSFUL;
            ret = Block4BytesWrite(0x7001, 0x0001);
            if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
            for (int n = 0; n < ElementDefine.RETRY_COUNTER; n++)
            {
                Thread.Sleep(20);
                ...
                ret = ElementDefine.IDS_ERR_DEM_RMP_FLAG;
            }
            ...
            for (int n = 0; n < ElementDefine.RETRY_COUNTER; n++)
            {
                ...
                ret = ElementDefine.IDS_ERR_DEM_EEPROM_RMP;
            }
            return ret;
        }
        ...
        private UInt32 BackUpFlash(ElementDefine.COBRA_NEWTON_MEMORY_TYPE type)
        {
            ...
            switch (type)
            {
                ...
                case ElementDefine.COBRA_NEWTON_MEMORY_TYPE.MEMORY_TYPE_PARAMETER:
                    ...
                    if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
                    Array.Copy(parent.ParameterPage_Buffer,(ElementDefine.ParameterArea_StartAddress - ElementDefine.ParameterPage_StartAddress*4), parent.m_ProjParamImg,0,parent.m_ProjParamImg.Length);
                    break;
            }
            //ret = ReLockI2C();
            return ret;
        ...
        private UInt32 ReStoreFlash(ElementDefine.COBRA_NEWTON_MEMORY_TYPE type)
        {
            ...
            switch (type)
            {
                case ElementDefine.COBRA_NEWTON_MEMORY_TYPE.MEMORY_TYPE_SyseFlash:
                    ...
                    return ret;
                case ElementDefine.COBRA_NEWTON_MEMORY_TYPE.MEMORY_TYPE_InfoeFlash:
                    WriteSysInfoAreaCleareFlashFreshBit(ref parent.InfoeFlash_Buffer);
        ...
        public UInt32 ChipOperation(ElementDefine.CHIP_OPERA_MODE mode)
        {
            ...
            switch (mode)
            {
                ...
                case ElementDefine.CHIP_OPERA_MODE.CPU_RUN:
                    ...
                    if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
                    break;
                case ElementDefine.CHIP_OPERA_MODE.CPU_WAKEUP:
                    ret = WriteWord(0x11, ElementDefine.UnLock_I2C_PSW);
                    if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;

                    ret = WriteWord(0x12, ElementDefine.I2C_AHB_MODE_Enable_PSW);
                    if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
        ...
        public UInt32 volCalib(ref TASKMessage msg)
        {
            ...
            uwval += SharedFormula.MAKEDWORD(SharedFormula.MAKEWORD(parent.ParameterPage_Buffer[imageAddr + 0], parent.ParameterPage_Buffer[imageAddr + 1]),
                SharedFormula.MAKEWORD(parent.ParameterPage_Buffer[imageAddr + 2], parent.ParameterPage_Buffer[imageAddr + 3]));
            ...
            parent.ParameterPage_Buffer[imageAddr + 0] = (byte)uwval;
            parent.ParameterPage_Buffer[imageAddr + 1] = (byte)(uwval >> 8);
            parent.ParameterPage_Buffer[imageAddr + 2] = (byte)(uwval >> 16);
            parent.ParameterPage_Buffer[imageAddr + 3] = (byte)(uwval >> 24);
        ...
        public UInt32 curCalib(ref TASKMessage msg)
        {
            ...
            lastSlope = SharedFormula.MAKEDWORD(SharedFormula.MAKEWORD(parent.ParameterPage_Buffer[imageAddr + 0], parent.ParameterPage_Buffer[imageAddr + 1]),
                SharedFormula.MAKEWORD(parent.ParameterPage_Buffer[imageAddr + 2], parent.ParameterPage_Buffer[imageAddr + 3]));
        ...
        private UInt32 EnableSTPMode()
        {
            UInt32 ret = LibErrorCode.IDS_ERR_SUCCESSFUL;
            ret = Block4BytesWrite(0x6040, 0x89ABCDEF);
            return ret;
        }
        #endregion
    }
