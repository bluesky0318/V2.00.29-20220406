...
namespace Cobra.KALL13D
{
    internal class DEMBehaviorManage
    {
        ...
        private const byte OP_FETCTRL = 0x5B;
        ...
        public UInt32 Write(ref TASKMessage msg)
        {
            ...
            if (EFUSEReglist.Count != 0)
            {
                ...
                ret = ReadWord(OP_FETCTRL, ref wdata);
                if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;

                ret = WriteWord(OP_FETCTRL, (UInt16)(wdata&0xFE));
                if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
                ...
                ret = WriteWord(OP_FETCTRL, wdata);
                if (ret != LibErrorCode.IDS_ERR_SUCCESSFUL) return ret;
            }
        ...
        public UInt32 ConvertHexToPhysical(ref TASKMessage msg)
        {
            ...
            if (EFUSEParamList.Count != 0)
            {
                ...
                param = EFUSEParamList.Find(p => p.guid == ElementDefine.EPROM_CellNumber);
                if (param == null) return LibErrorCode.IDS_ERR_SUCCESSFUL;
                if(param.errorcode == ElementDefine.IDS_ERR_DEM_CELLNUMBER_OVERRANGE)
                {
                    param.errorcode = LibErrorCode.IDS_ERR_SUCCESSFUL;
                    return ElementDefine.IDS_ERR_DEM_CELLNUMBER_OVERRANGE;
                }
            }
